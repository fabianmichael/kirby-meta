...
<main class="main" id="meta-debug">
    <article class="o-wrap">
      <?php snippet('hero', ['align' => 'center']) ?>
      <div class="text">
        

        <button type="button" id="meta-check-links" class="c-button">Check for broken links (and grab some ☕️)!</button>
        <pre id="meta-check-status">–</pre>
        <br><br>

        <table id="meta-debug-table" style="border: 1px solid white; border-collapse: collapse;">
            <thead style="position: sticky; top: 0; font-size: 11px; background: #eee; z-index: 5">
                <tr>
                    <th style="vertical-align: bottom;" width="100">Template</th>
                    <th style="vertical-align: bottom;">OG title</th>
            ...
            <tbody>
                ...
                foreach (site()->index() as $item) {
                    ...
                    echo '<tr class="meta-debug-header--' . $item->id() . '">';
                        echo '<th colspan="8" style="background: #eee">' . $item->title()->link() . ' <code class="meta-debug-page-id">' . $item->id() . '</code></th>';
                    echo '</tr>';
                    ...
                    echo '<td>' . (($thumbnail = $meta->thumbnail()) ? $thumbnail->resize(100) : '❌') . '</td>';
